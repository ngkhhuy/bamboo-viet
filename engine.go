...
func (e *IBusBambooEngine) SetSurroundingText(text dbus.Variant, cursorPos uint32, anchorPos uint32) *dbus.Error {
	defer func() {
		if err := recover(); err != nil {
			fmt.Println("Recovered in SetSurroundingText:", err)
		}
	}()
	...
	if rVal.Kind() == reflect.Slice && rVal.Len() >= 3 {
		str = reflect.ValueOf(rVal.Index(2).Interface()).String()
	} else if rVal.Kind() == reflect.String {
		str = rVal.String()
	}

	// Handle active selection (such as inline autocomplete in Address Bar / Search Box)
	...
	if int(effectivePos) > len(s) {
		effectivePos = uint32(len(s))
	}
	cs := s[:effectivePos]

	// Extract only the current word before cursor (up to the nearest word break)
	startIdx := 0
	...
	e.currentWordNearCursor = string(wordRunes)
	...
	if e.inBackspaceWhiteList() {
		...
		for i := len(wordRunes) - 1; i >= 0; i-- {
			e.preeditor.ProcessKey(wordRunes[i], bamboo.EnglishMode|bamboo.InReverseOrder)
		}
	}
	return nil
}
