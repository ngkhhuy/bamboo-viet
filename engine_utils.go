...
func (e *IBusBambooEngine) getInputMode() int {
	...
	if wmClass != "" {
		// 1. User manual mapping override
		if im, ok := e.config.InputModeMapping[wmClass]; ok && config.ImLookupTable[im] != "" {
			return im
		}
		// 2. Built-in optimal app preset resolution
		if presetIM, ok := config.ResolvePresetInputMode(wmClass); ok && config.ImLookupTable[presetIM] != "" {
			return presetIM
		}
	}
